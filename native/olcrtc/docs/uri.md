...
## Внутренний формат Olcrtc_manager

Admin UI панели `Olcrtc_manager` генерирует URI в собственном формате, отличном от описанного выше. Этот формат используется внутренним клиентом `olcrtc_client` и **не совместим** со сторонними клиентами (Olcbox и др.).

### Формат

```text
olcrtc://<Auth>@r/<RoomID>?k=<EncryptionKey>&t=<Transport>&f=<FPS>&b=<Batch>&core=legacy&c=<ClientID>#<Name>
```

### Отличия от спецификации

| Элемент | Спецификация | Manager (внутренний) |
|---------|-------------|---------------------|
| Транспорт | после `?` как имя | query-параметр `&t=` |
| Параметры транспорта | `<key=value>` в угловых скобках | отдельные query-параметры (`&f=`, `&b=`) |
| Ключ шифрования | после `#` | query-параметр `?k=` |
| Room ID | после `@` напрямую | с префиксом `@r/` |
| MIMO (`$`) | поддерживается | отсутствует |

### Переключение форматов

В Admin UI доступны оба формата:
- **URI** — внутренний формат manager'а (для `olcrtc_client`)
- **Spec URI** — формат по настоящей спецификации (для сторонних клиентов)

API-эндпоинты:
- `GET /api/instances/{id}/uri` — внутренний формат
- `GET /api/instances/{id}/spec-uri` — формат по спецификации
- `GET /api/instances/{id}/qr` — QR во внутреннем формате
